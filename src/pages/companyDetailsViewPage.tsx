...
function CompanyDetailsViewPage({ company, setView, updateCompany, language }: CompanyDetailsViewPageProps) {
            ...
            <div className="flex flex-col flex-grow">
                <div className="flex justify-between items-center p-3 bg-white shadow-sm mb-3 rounded-lg">
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={() => setView("company-visualizations")}
                            ...
                            aria-label={t["View Visualizations"] || "View Visualizations"}
                        >
                            {t["View Visualizations"] || "View Visualizations"}
                        </button>
                        <button
                            onClick={() => setView("company-comparison")}
                            ...
                            aria-label={t["Compare Portfolio"] || "Compare Portfolio"}
                        >
                            {t["Compare Portfolio"] || "Compare Portfolio"}
                        </button>
                        <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                    <div className="bg-white p-3 rounded-lg shadow-sm">
                        <h2 className="text-sm font-medium text-gray-900">{t["Total Carrying Amount"] || "Total Carrying Amount"}</h2>
                        <p className="text-xl text-gray-700">{summary.totalCarryingAmount ? `$${summary.totalCarryingAmount.toLocaleString()}` : t["Zero Value"]}</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg shadow-sm">
                        <h2 className="text-sm font-medium text-gray-900">{t["Total Fair Value"] || "Total Fair Value"}</h2>
                        <p className="text-xl text-gray-700">{summary.totalFairValue ? `$${summary.totalFairValue.toLocaleString()}` : t["Zero Value"]}</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg shadow-sm">
                        <h2 className="text-sm font-medium text-gray-900">{t["Ownership by Type"] || "Ownership by Type"}</h2>
                ...
                <div
                    ...
                    style={{ height: "calc(100vh - 64px - 80px - 104px)" }}
                >
                    <div
                    ...
                    >
                        {filteredData.length === 0 ? (
                            <div className="p-4 text-gray-600 text-center">{t["No holdings match the current filters"] || "No holdings match the current filters"}</div>
                        ) : (
